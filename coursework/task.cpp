...
public:
    ...
    PropertyOwner(const PropertyOwner &other) : Person(other), n(other.n)
    {
        for (int i = 0; i < MAXP; i++)
            props[i] = nullptr;
        for (int i = 0; i < n; i++)
            props[i] = copy_cstr(other.props[i]);
    }
    ...
    bool addressMatchesAnyProperty() const
    {
        for (int i = 0; i < n; i++)
        {
            if (props[i] && strcmp(getAddress(), props[i]) == 0)
                return true;
        }
        return false;
    }
...
int main()
{
    ...
    while (true)
    {
        ...
        {
            if (saveToFile(filename, people))
                cout << "Записано в " << filename << "\n";
            else
                cout << "Грешка при запис!\n";
        }
        else
        {
